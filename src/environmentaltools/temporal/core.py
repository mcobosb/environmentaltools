...
def transform(data: pd.DataFrame, params: dict):
    ...
    Transforms data using Box-Cox or Yeo-Johnson power transformations to
    make the distribution more Gaussian-like. Useful for stabilizing variance
    and improving model fit.

    Parameters
    ----------
    data : pd.DataFrame or pd.Series
        Input time series data to transform. If Series, converts to DataFrame.
    params : dict
        ...
        - 'lambda' : float, optional
            Transformation parameter. If provided, uses this value; 
            otherwise computes optimal lambda via MLE
    ...
    params : dict
        Updated params dictionary with 'transform']['lambda'] set to the
    ...
    Transformation does NOT standardize (mean=0, std=1); only applies the
    power transformation.

    See Also
    --------
    inverse_transform : Reverse the transformation
    sklearn.preprocessing.PowerTransformer : Underlying implementation
...
def inverse_transform(data: pd.DataFrame, params: dict, ensemble: bool = False):
    ...
    Applies inverse of Box-Cox or Yeo-Johnson transformation to return
    data to its original scale after analysis in transformed space.

    Parameters
    ----------
    data : pd.DataFrame
        Transformed data to convert back to original scale
    params : dict
        Parameter dictionary containing transformation information:

        - If ensemble=False: uses params['transform']['method'] and
          params['transform']['lambda']
        - If ensemble=True: uses params['method_ensemble'] and
          params['lambda_ensemble']
            
    ensemble : bool, default=False
        Whether to use ensemble transformation parameters instead of 
    ...
    This function must use the same lambda value that was used in the 
    forward transform() call to ensure correct inversion.

    See Also
    --------
    transform : Forward power transformation
    sklearn.preprocessing.PowerTransformer : Underlying implementation
